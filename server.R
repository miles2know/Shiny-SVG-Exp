...
p <- ggplot(chooseWedge(), aes(x = Year, y = Ghg, group = Sector)) +
      stat_smooth(mapping = aes(fill = Sector), geom="area", position="stack", method="gam", formula = y~s(x)) +
      scale_fill_manual(values = colors) +  annotate("text", x = 2016, y = 122, label = "2030 Extrpolation to 2050 Target") +
      ...
      geom_hline(yintercept=127, color = "black", size = 1, linetype = 5 ) + geom_hline(yintercept=38.28, color = "black", size = 1, linetype = 5 ) +
